            ...
            when (msg.what) {
                ...
                Constants.SERVICE_WHAT.STOPWATCH_RESET, Constants.SERVICE_WHAT.TIMER_RESET -> {
                    if (mTimerTaskMSec > 0) {
                        ...
                        val materialDialog = MaterialDialog.Builder(mServiceContext!!)
                                ...
                                .onPositive { dialog, which -> mPlankCallback!!.savePlankLog(method) }
                                .dismissListener { resetService(method) }
                                .cancelable(false)
                                .build()

                        materialDialog.window!!.setType(WindowManager.LayoutParams.TYPE_SYSTEM_ALERT)
                        materialDialog.show()
                    } else {
                        resetService(method)
                    }
